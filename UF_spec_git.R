...
##Simulate DCE rankings (for open-sourcing)

###Fictional antibiotics
fictabs <- c("Abelfenide","Acetemran","Adenomadin","Adrevenac","Amrodine",    
             "Choriotroban","Cormide","Decloxone","Dexaset","Endoxolol",   
             "Olanzasys","Pansolid","Protestryl")
rankopts <- as.list(seq(1,length(fictabs)))
probvals <- c(0.25,0.2,0.1,0.1,0.05,0.05,0.05,0.04,0.03,0.03,0.04,0.04,0.02)
names(rankopts) <- sample(probvals,replace=F)

###Specialties and number of participants
specialties <- c("General Practice","Intensive care","Medicine","Surgery",
                 "Infection")
n_participants <- 49

###Assemble ranking dataframe
rankings <- data.frame(matrix(ncol=length(fictabs),nrow=n_participants))
for (i in 1:n_participants){
  rankopts_list <- rankopts
  
  for (j in rankopts){
    
    rankindex <- sample(seq(1:length(rankopts_list)),1,prob=names(rankopts_list)) %>% unlist() 
    
    rankings[i,j] <- rankopts_list[rankindex] %>% unlist()
    
    rankopts_list <- rankopts_list[-rankindex]
    
  }
  
}
colnames(rankings) <- fictabs

###Add specialties
rankings$Specialty <- sample(specialties,n_participants,replace=T)
