...
stabmet_df <- read_csv("/Users/alexhoward/Documents/Projects/UDAST_code/stabmet_df.csv")
timemet_df <- read_csv("/Users/alexhoward/Documents/Projects/UDAST_code/timemet_df.csv")
fairness_writedf <- read_csv("/Users/alexhoward/Documents/Projects/UDAST_code/fairness_writedf.csv")
...
Urine culture isolates were susceptible to AUF and human antibiotic choices with similar frequency `r chi_all`. The AUF chose more appropriate Access  `r chi_acs` and oral antibiotics `r chi_orals` than human prescribers, but fewer appropriate intravenous antibiotics `r chi_ivs`. The AUF testing panel provided more susceptible results per specimen than the standard panel `r wil_all`, including for Access antibiotics `r wil_acs` and intravenous antibiotics `r wil_iv` but not oral antibiotics `r wil_po`.
...
\
When model stability was assessed by progressively decreasing training dataset size (Appendix 11), the `r stabmet_df %>% select(top_stab_amdmod) %>% unlist()` model had the largest difference in mean AUROC across six random train-test splits (`r stabmet_df %>% select(top_stab_amdtop) %>% unlist()` with `r stabmet_df %>% select(top_stab_amdtoplab) %>% mutate(top_stab_amdtoplab=top_stab_amdtoplab*100) %>% unlist()`% training:`r 100-(stabmet_df %>% select(top_stab_amdtoplab) %>% mutate(top_stab_amdtoplab=top_stab_amdtoplab*100) %>% unlist())`% testing, versus `r stabmet_df %>% select(top_stab_amdbot) %>%unlist()` with `r stabmet_df %>% select(top_stab_amdbotlab) %>% mutate(top_stab_amdbotlab=top_stab_amdbotlab*100) %>% unlist()`% training:`r 100-(stabmet_df %>% select(top_stab_amdbotlab) %>% mutate(top_stab_amdbotlab=top_stab_amdbotlab*100) %>% unlist())`% testing). The `r stabmet_df %>% select(top_stab_asdmod) %>% unlist()` susceptibility prediction model had the largest AUROC standard deviation (`r stabmet_df %>% select(top_stab_asdval) %>% unlist()` with `r stabmet_df %>% select(top_stab_asdcase) %>% mutate(top_stab_asdcase=top_stab_asdcase*100) %>% unlist()`% training:`r 100-(stabmet_df %>% select(top_stab_asdcase) %>% mutate(top_stab_asdcase=top_stab_asdcase*100) %>% unlist())`% testing). When models were tested out-of-sample by training on data from one time period and testing on data from other time periods (Appendix 13), the `r timemet_df %>% select(top_time_amdmod) %>% unlist()` prediction model had the largest difference in mean AUROC between training and testing time groups (`r timemet_df %>% select(top_time_amdtop) %>% unlist()` with `r timemet_df %>% select(top_time_amdtoptrain) %>% unlist() %>% str_remove_all(" ")` training:`r timemet_df %>% select(top_time_amdtoptest) %>% unlist() %>% str_remove_all(" ")` testing versus `r timemet_df %>% select(top_time_amdbot) %>% unlist()` with `r timemet_df %>% select(top_time_amdbottrain) %>% unlist() %>% str_remove_all(" ")` training:`r timemet_df %>% select(top_time_amdbottest) %>% unlist() %>% str_remove_all(" ")` testing) and the `r timemet_df %>% select(top_time_asdmod) %>% unlist()` prediction model also had the largest AUROC standard deviation (`r timemet_df %>% select(top_time_asdval) %>% unlist()` with `r timemet_df %>% select(top_time_sdtrainlab) %>% unlist() %>% str_remove_all(" ")` training:`r timemet_df %>% select(top_time_sdtestlab) %>% unlist() %>% str_remove_all(" ")` testing). When model fairness was assessed by measuring predictive performance in different demographic groups (Appendix 14), the largest differences in mean AUROC were observed when predicting the antibiotic susceptibility of urine culture isolates for different age groups (`r fairness_writedf %>% filter(char=="age group") %>% select(top_auc) %>% unlist()` for `r fairness_writedf %>% filter(char=="age group") %>% select(top_aucmodel) %>% unlist()` in `r fairness_writedf %>% filter(char=="age group") %>% select(top_auclab) %>% unlist()`s versus `r fairness_writedf %>% filter(char=="age group") %>% select(bot_auc) %>% unlist()` in `r fairness_writedf %>% filter(char=="age group") %>% select(bot_auclab) %>% unlist()`s), races (`r fairness_writedf %>% filter(char=="race") %>% select(top_auc) %>% unlist()` for `r fairness_writedf %>% filter(char=="race") %>% select(top_aucmodel) %>% unlist()` in `r fairness_writedf %>% filter(char=="race") %>% select(top_auclab) %>% unlist()` patients versus `r fairness_writedf %>% filter(char=="race") %>% select(bot_auc) %>% unlist()` in `r fairness_writedf %>% filter(char=="race") %>% select(bot_auclab) %>% unlist()` patients), and sexes (`r fairness_writedf %>% filter(char=="gender") %>% select(top_auc) %>% unlist()` for `r fairness_writedf %>% filter(char=="gender") %>% select(top_aucmodel) %>% unlist()` in `r fairness_writedf %>% filter(char=="gender") %>% select(top_auclab) %>% unlist() %>% tolower()`s versus `r fairness_writedf %>% filter(char=="gender") %>% select(bot_auc) %>% unlist()` in `r fairness_writedf %>% filter(char=="gender") %>% select(bot_auclab) %>% unlist() %>% tolower()`s). The largest AUROC standard deviations were observed when making predictions for `r fairness_writedf %>% filter(char=="age group") %>% select(top_sdlab) %>% unlist()`s (`r fairness_writedf %>% filter(char=="age group") %>% select(top_sd) %>% unlist()` for `r fairness_writedf %>% filter(char=="age group") %>% select(top_sdmodel) %>% unlist()`), `r fairness_writedf %>% filter(char=="race") %>% select(top_sdlab) %>% unlist()` patients (`r fairness_writedf %>% filter(char=="race") %>% select(top_sd) %>% unlist()` for `r fairness_writedf %>% filter(char=="race") %>% select(top_sdmodel) %>% unlist()`), and `r fairness_writedf %>% filter(char=="gender") %>% select(top_sdlab) %>% unlist() %>% tolower()`s (`r fairness_writedf %>% filter(char=="gender") %>% select(top_sd) %>% unlist()` for `r fairness_writedf %>% filter(char=="gender") %>% select(top_sdmodel) %>% unlist()`).
...
The antimicrobial susceptibility testing panel composed of the top six AUF choices (Appendix 17) provided more susceptible results per specimen than the standard panel `r wil_all`, including more Access `r wil_acs` and intravenous antibiotics `r wil_iv` but similar numbers of oral antibiotics `r wil_po`.
