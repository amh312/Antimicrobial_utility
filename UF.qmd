...
chi_all <- glue("({perc_all1}% of cases versus {perc_all2}%, 95% confidence interval of difference [CI] {ci1_all}% to {ci2_all}%, p{ifelse(p2_all<0.001,'<0.001',paste0('=',p2_all))})")
...
chi_acs <- glue("({perc_acs1}% versus {perc_acs2}%, CI {ci1_acs}% to {ci2_acs}%, p{ifelse(p2_acs<0.001,'<0.001',paste0('=',p2_acs))})")
...
chi_orals <- glue("({perc_orals1}% versus {perc_orals2}%, CI {ci1_orals}% to {ci2_orals}%, p{ifelse(p2_orals<0.001,'<0.001',paste0('=',p2_orals))})")
...
chi_ivs <- glue("({perc_ivs1}% versus {perc_ivs2}%, CI {ci1_ivs}% to {ci2_ivs}%, p{ifelse(p2_ivs<0.001,'<0.001',paste0('=',p2_ivs))})")
...
Urine culture isolates were more frequently susceptible to AUF antibiotic choices than human antibiotic choices `r chi_all`. The AUF recommended more appropriate Access  `r chi_acs` and oral antibiotics `r chi_orals` but fewer appropriate intravenous antibiotics `r chi_ivs`. The AUF testing panel provided more susceptible results per specimen than the standard panel `r wil_all`, including more Access `r wil_acs`, oral `r wil_po`, and intravenous antibiotics `r wil_iv`.
...
Funded in part by the Wellcome Trust (226691/Z/22/Z). Supported by the Office for Life Sciences Data-Action Accelerator.
