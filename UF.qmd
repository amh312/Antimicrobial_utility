...
Performance metrics from the small training dataset stability analysis are summarised in Appendix 8. The largest difference in mean AUROC across six model train-test iterations was observed between the 2% and 12% training datasets in the CDI prediction model (0.132, 0.631 versus 0.763), and the largest AUROC standard deviation was observed in the 2% dataset in the CDI prediction model (0.025). Performance metrics from the year group cluster analyses are summarised in Appendix 9. For all models and all year groups, mean AUROC across six train-test iterations was highest in the same year group upon which the model had been trained. The largest difference in AUROC between year groups was observed between [YEAR GROUP Y] and [YEAR GROUP Z] in the [MODEL] trained on [YEAR GROUP X] ([X, Y versus Z]). The largest AUROC standard deviation was observed when THE [MODEL] trained on [YEAR GROUP X] was tested on [YEAR GROUP Y] (X).

Performance metrics from the model fairness analyses are summarised in Appendix 10. The largest difference in mean AUROC across six model train-test iterations in the age fairness analysis was observed between [AGE-X] and [AGE-Y] for [MODEL X] ([X, Y versus Z]), and the largest AUROC standard deviation in [AGE-Z] for [MODEL Y] ([X]). The largest difference in mean AUROC across six model training iterations in the race fairness analysis was observed between [RACE-X] and [RACE-Y] for [MODEL X] ([X, Y versus Z]), and the largest AUROC standard deviation in [RACE-Z] for [MODEL Y] ([X]). The largest difference in mean AUROC across six model training iterations in the remaining characteristics was observed between females and males - this difference was most marked in susceptibility prediction models for cephalosporins, meropenem, and gentamicin, with the largest difference in mean AUROC observed for ([ANTIBIOTIC-X]) ([X, Y versus Z]), and the largest AUROC standard deviation in [ANTIBIOTIC-Y] ([X]).

## Discrete choice experiment

The coefficients from the ranked logit analysis of the discrete choice experiment across all specialties are displayed in Figure 3, and the coefficients by individual specialty are displayed in Appendix 11.
...
Record the results of the discrete choice experiment.

## Utility function

The distributions of single-antimicrobial choice utility values for patients across the microsimulation dataset are displayed in Figure 2, and the distributions of all antimicrobial choice utility values (including 2-agent combinations) both overall and stratified by prescriber types are displayed in Appendix 12.
...
Report the utility values and sensitivity analysis for the utility function.
