...
##Read-in
ur_util <- read_csv("#FILEPATH#/ur_util_manuscript.csv")
util_probs_df <- read_csv("#FILEPATH#/util_probs_df_manuscript.csv")
desc_tab <- read_csv("#FILEPATH#/uf_desctab.csv")
urines5 <- read_csv("#FILEPATH#/urines5.csv")
abx <- read_csv("#FILEPATH#/interim_abx.csv")
ab_tab <- read_csv("#FILEPATH#/ab_tab.csv")
metrics_singles_table <- read_csv("#FILEPATH#/metrics_singles_table.csv")
prim <- read_csv("#FILEPATH#/overall_xg_plot_df.csv")
pdrx <- read_csv("#FILEPATH#/abplot_df.csv")
pdrx_px_abx <- read_csv("#FILEPATH#/abplot_df_px_abx.csv")
cdi_df<- read_csv("#FILEPATH#/cdi_df.csv")
tox_df<- read_csv("#FILEPATH#/tox_df.csv")
cost_df<- read_csv("#FILEPATH#/cost_df.csv")
cdi_df_px_abx<- read_csv("#FILEPATH#/cdi_df_px_abx.csv")
tox_df_px_abx<- read_csv("#FILEPATH#/tox_df_px_abx.csv")
cost_df_px_abx<- read_csv("#FILEPATH#/cost_df_px_abx.csv")
stabmets <- read_csv("#FILEPATH#/overall_stability_metrics.csv")
timemets <- read_csv("#FILEPATH#/overall_time_sens_metrics.csv")
fairmets <- read_csv("#FILEPATH#/overall_fairness_metrics.csv")
overall_xg_plot_df <- read_csv("#FILEPATH#/overall_xg_plot_df.csv")
overall_xg_plot_df_px_abx <- read_csv("#FILEPATH#/overall_xg_plot_df_px_abx.csv")
...
chi_all <- glue("({perc_all1}% of cases versus {perc_all2}%, 95% confidence interval of difference [CI] {ci1_all}% to {ci2_all}%, p{ifelse(p2_all<0.001,'<0.001',paste(' =',p2_all))})")
...
chi_acs <- glue("({perc_acs1}% versus {perc_acs2}%, CI {ci1_acs}% to {ci2_acs}%, p{ifelse(p2_acs<0.001,'<0.001',paste(' =',p2_acs))})")
...
chi_orals <- glue("({perc_orals1}% versus {perc_orals2}%, CI {ci1_orals}% to {ci2_orals}%, p{ifelse(p2_orals<0.001,'<0.001',paste(' =',p2_orals))})")
...
chi_ivs <- glue("({perc_ivs1}% versus {perc_ivs2}%, CI {ci1_ivs}% to {ci2_ivs}%, p{ifelse(p2_ivs<0.001,'<0.001',paste(' =',p2_ivs))})")
...
wil_all <- glue("(median {m_aut_all}, interquartile range [IQR] {iq1aut_all}-{iq3aut_all} versus median {m_stan_all}, IQR {iq1stan_all}-{iq3stan_all}, p{ifelse(p_all<0.001,'<0.001',paste(' =',p_all))}, effect size [ES] {all_effectsize})")

wil_acs <- glue("(median {m_aut_acs}, IQR {iq1aut_acs}-{iq3aut_acs} versus median {m_stan_acs}, IQR {iq1stan_acs}-{iq3stan_acs}, p{ifelse(p_acs<0.001,'<0.001',paste(' =',p_acs))}, ES {acs_effectsize})")

wil_po <- glue("(median {m_aut_pos}, IQR {iq1aut_pos}-{iq3aut_pos} versus median {m_stan_pos}, IQR {iq1stan_pos}-{iq3stan_pos}, p{ifelse(p_po<0.001,'<0.001',paste(' =',p_po))}, ES {po_effectsize})")

wil_iv <- glue("(median {m_aut_ivs}, IQR {iq1aut_ivs}-{iq3aut_ivs} versus median {m_stan_ivs}, IQR {iq1stan_ivs}-{iq3stan_ivs}, p{ifelse(p_iv<0.001,'<0.001',paste(' =',p_iv))}, ES {iv_effectsize})")
...
To improve the precision of antibiotic treatment using data-driven decision support tools, developers need to quantify the utility of different antibiotic treatment options. Here, we report a utility function that recommends antibiotic treatments and susceptibility tests for urinary tract infection by combining machine learning predictions and prescriber knowledge. 
...
knitr::include_graphics("#FILEPATH#/wk_plot.pdf")
...
knitr::include_graphics("#FILEPATH#/roc_plots_main.pdf")
  
```

### Figure 2: Receiver operating characteristic curves for probability predictions made for antibiotic susceptibility, *C. difficile* infection (CDI), and toxicity on the validation dataset.
\
```{r, echo=FALSE,message=FALSE,warning=FALSE}
...
\
When the stability of the model was assessed by progressively decreasing training dataset size (Appendix 9), the CDI model had the largest difference in mean AUROC across six random train-test splits (0.667 with 2% training:98% testing, versus 0.776 with 14% training:86% testing). The vancomycin susceptibility prediction model had the largest AUROC standard deviation (0.025 with 2% training:98% testing). When the model was tested out-of-sample by training on one time period and testing on others (Appendix 10), the toxicity prediction model had the largest difference in mean AUROC between training and testing time groups (0.897 with 2017-2019 training:2017-2019 testing versus 0.654 with 2017-2019 training:2014-2016 testing). The CDI prediction model had the largest AUROC standard deviation (0.078 with 2014-2016 training:2017-2019 testing). When model fairness was assessed by testing predictive accuracy in different demographic groups (Appendix 11), the largest differences in mean AUROC were observed when predicting the antibiotic susceptibility of urine culture isolates for different age groups (0.826 for piperacillin-tazobactam in 40-49s versus 0.632 in 30-39s), races (0.716 for ciprofloxacin in Hispanic patients versus 0.623 in the aggregated least prevalent races), and sexes (0.725 for meropenem in females versus 0.603 in males). The largest AUROC standard deviations were observed between ages, races, and sexes when predicting piperacillin-tazobactam susceptibility (0.113 in 30-39s, 0.089 in Asian patients, 0.022 in males).
...
knitr::include_graphics("#FILEPATH#/ORplot.pdf")
...
```{r, echo=FALSE,message=FALSE,warning=FALSE}
knitr::include_graphics("#FILEPATH#/utility_Treatment_ (single agent).pdf")
```
...
\
```{r, echo=FALSE,message=FALSE,warning=FALSE}
knitr::include_graphics("#FILEPATH#/illness_abplot.pdf")
knitr::include_graphics("#FILEPATH#/illness_abplot_px_abx.pdf")
...
The antimicrobial susceptibility testing panel composed of the top six algorithmic choices (Appendix 14) provided more susceptible results per specimen than the standard panel `r wil_all`, including more Access `r wil_acs`, oral `r wil_po`, and intravenous antibiotics `r wil_iv`.
