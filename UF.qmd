...
Imprecise antibiotic use is a key driver of antimicrobial resistance (AMR). The World Health Organisation (WHO) has therefore set a target for countries to reduce their use of broad-spectrum antibiotics by 2030. Meeting this target is made challenging by the tension between maximising antibiotic effectiveness in patients at high risk of poor infection outcomes and minimising harms to patients and populations from antibiotic exposure. Optimal antibiotic treatment and testing decisions could be learned from healthcare data using machine learning algorithms, but these require utility functions to quantify the values of different options. 
...
The utility function we report here could underpin decision support tools – leveraging data, machine learning, and domain expertise to help clinicians, healthcare providers, and countries reduce medium- and long-term antibiotic harm to individuals and populations without compromising treatment efficacy. Further clinical studies with utility function-based antibiotic decision support tools are required across a range of populations and care contexts to better understand their impacts on individual and population outcomes. 

# Abstract

## Background

To improve antibiotic treatment precision with data-driven decision support tools, developers need to quantify the utility of different antibiotic treatment options. Here, we report a utility function that recommends antibiotic treatments for urinary tract infection by combining machine learning predictions and prescriber knowledge. 

## Methods

The utility function retrospectively recommended personalised treatment choices and six-antibiotic susceptibility testing panels for `r nrow(ur_util) %>% comma()` patients using healthcare data available when urine cultures were sent. The appropriateness of each testing and treatment recommendation was compared to the human prescriber treatment choice and a standard testing panel respectively using the subsequent urine antibiotic susceptibility result. The proportions of appropriate recommendations that were World Health Organisation Access category, oral, and/or intravenous antibiotics were measured.

## Findings

Urine culture isolates were susceptible to the antibiotic chosen by the utility function more often than they were susceptible to antibiotics chosen by human prescribers `r chi_all` — more of these appropriate treatment choices were WHO Access category `r chi_acs` and orally-administrable `r chi_orals` than human prescriber choices, but fewer were intravenously administrable `r chi_ivs`. The utility function antimicrobial susceptibility testing panel provided more susceptible results per specimen than the standard panel `r wil_acs`, including more Access `r wil_all`, oral `r wil_po`, and intravenous antibiotics `r wil_iv`.

## Interpretation

The utility function described here could improve antibiotic treatment appropriateness by balancing risks of antibiotic failure against harm. Utility function-based decision support tools need evaluating in a diversity of populations and clinical contexts.
...
### Figure 2: Receiver operating characteristic curves for probability predictions made for single-antibiotic susceptibility, *C. difficile* infection (CDI), and toxicity on the validation dataset.
\
```{r, echo=FALSE,message=FALSE,warning=FALSE}
