...
Clinical prediction model training hyperparameters and predictor variable contributions are summarised in Appendixes 7 and 8 respectively. Resistance to an antibiotic in the last year, gender and age group were large contributors to predicting antibiotic activity against the urine pathogen. Hospital admission or acute kidney injury in the last year, age ≥65, and previous CDI were large contributors to predicting CDI and antibiotic toxicity. Validation receiver operating characteristic (ROC) curves and performance metrics for the 15 clinical prediction models (one predicting susceptibility to each antimicrobial agent, one CDI, and one drug toxicity) are summarised in Figure 2 and Table 1 respectively. Eight models produced AUROCs between 0.7 and 0.8, five between 0.65 and 0.7. The CDI model had the highest AUROC, at `r aucfilter(metrics_singles_table,"CDI")`, and the model predicting ampicillin activity against the urine pathogen had the lowest, at `r aucfilter(metrics_singles_table,"Ampicillin")`.
...
\
The proportions of urinary pathogens that were susceptible to utility function treatment recommendations and actual ED prescriptions for patients with different illness severity scores are displayed in Figure 5. The recommendations themselves are displayed in Figure 6.  Utility function-based algorithms chose more antibiotic treatments that were active against urinary pathogens than human prescribers did `r chi_all`, including more active Access `r chi_acs` and oral antibiotics `r chi_orals`, but fewer active intravenous antibiotics `r chi_ivs`. As illness severity rose, nitrofurantoin was initially deprioritised mainly in favour of gentamicin (likely due to its intravenous route of administration), which was in turn deprioritised for the Watch antibiotic piperacillin-tazobactam (likely due to its increased probability of covering the urinary pathogen). ED prescriptions exhibited a similar shift towards intravenous antibiotics as illness severity rose, but favoured Watch antibiotics (mainly ceftriaxone) over gentamicin at intermediate illness severities, resulting in higher rates of Watch agent use overall. Overall rates of antibiotic activity were similar in algorithmic recommendations (`r glue("{round(perc_util,1)}%")`) to those in actual ED prescriptions (`r glue("{round(perc_actual,1)}%")`) at the highest illness severity. The antimicrobial susceptibility testing panel composed of the top six algorithmic choices (Appendix 14) contained more antibiotics per specimen that were active against urinary pathogens than the standard panel `r wil_acs`, including more Access `r wil_all` and intravenous antibiotics `r wil_iv`, but similar numbers of oral antibiotics `r wil_po`.\
...
```
### Figure 5: Plots displaying how activity of simulated empirical algorithmic treatment recommendations (left) against urinary pathogens changed with increasing illness severity scores, compared with empirical antibiotics actually prescribed by ED clinicians for the same patients (right). Red areas demonstrate the proportion of cases in which the empirical antibiotic was active against the urinary pathogen that was subsequently grown, and blue areas demonstrate the proportion of these antibiotics that were Access (top), oral (middle), and intravenous (bottom) antibiotics. The UNGA 70% Access agent target is displayed as a green dotted line on the top plots.
\
```{r, echo=FALSE,message=FALSE,warning=FALSE}
...
This study suggests that decision support systems based on our utility function could improve the appropriateness of empirical antibiotic prescribing for UTI. Human prescribers understand the importance of different antibiotic outcomes better than algorithms, but are less good at estimating their probability — our algorithmic approach solves this problem by algorithmically combining human judgement (quantitative analysis of the discrete choice experiment) with data-driven probability predictions (machine learning).[@Li2020] The utility function also manages the tension between the need for individual efficacy and the protection of antibiotics as societal assets using a safety mechanism — as illness severity increases, the algorithm recommends more intravenous and Watch category agents to maximise the probability of efficacy. Our results demonstrate that this approach could increase the use of Access category agents — implementation of decision support systems based on the utility function could therefore help countries meet the UN General Assembly target of 70% Access antibiotic use by 2030.

The results of our prescriber discrete choice experiment demonstrate that different clinicians have different priorities that reflect the needs of their patient cohorts. This is likely to also to apply to different healthcare settings (e.g., high versus low- and middle-income). The utility function is constructed in a way that allows each specialty in each healthcare jurisdiction to have its own locally-adapted utility function that reflects these needs, through two mechanisms: firstly, the discrete choice experiment can be undertaken by clinicians in the area it is to be used, ensuring importance weights $w$ are appropriate to the local setting; and secondly, the safety mechanism can be calibrated to the local illness severity score of choice (e.g., APACHE-2 instead of NEWS in ICU). The recommendations could also algorithmically account for antibiotic allergies and contraindications (not done in this study due to a lack of allergy information). Digital infrastructure may be a barrier to personalised recommendations in LMICs, but not to population and policy-level decisions (e.g., antibiotic formularies and supply chains).

The microsimulation approach gauges the potential clinical performance of algorithms in a more meaningful way than metrics such as AUROC, but a decision support tool based on the utility function should be further validated in interventional clinical studies where it can be directly compared against a standard of care (e.g., antimicrobial formulary). We hypothesise based on our microsimulation results that the introduction of the tool in such studies could increase the proportion antibiotics used for UTI that are Access category and orally-administered without increasing attendance, length of stay, or mortality. The studies should be performed in different populations with different AMR rates to ensure observed differences in clinical prediction model performance between demographic groups, time windows, and training dataset sizes do not affect outcomes. The performance of the algorithm in rare/unexpected situations (e.g., carbapenem resistance) also needs to be assessed.

Robust, scalable implementation of decision support tools based on the utility function in healthcare will require several considerations to be made: firstly, local dataflows will need to be complete and timely enough to inform clinical prediction models at individual (if available) or population level, depending on the application; secondly, a local hub will be required for the delivery, maintenance, and performance monitoring of decision support to clinicians — we propose that the microbiology laboratories could form these hubs, given that they already house infection domain experts and the infrastructure to communicate results to clinicians; thirdly, regulatory requirements for clinical decision support tools need to allow algorithms to adapt better to local populations — one of the main benefits of our utility functions is its ability to adapt to the priorities and needs of local clinicians and patients.
